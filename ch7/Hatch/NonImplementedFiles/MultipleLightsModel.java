...
public Model(GL3 gl, Camera camera, ArrayList<Light> lightArray, Shader shader, Material material, Mat4 modelMatrix, Mesh mesh, int[] textureId1, int[] textureId2) {
 this.mesh = mesh;
...
public Model(GL3 gl, Camera camera, ArrayList<Light> lightArray, Shader shader, Material material, Mat4 modelMatrix, Mesh mesh, int[] textureId1) {
 this(gl, camera, lightArray, shader, material, modelMatrix, mesh, textureId1, null);
}
public Model(GL3 gl, Camera camera, ArrayList<Light> lightArray, Shader shader, Material material, Mat4 modelMatrix, Mesh mesh) {
...
public void render(GL3 gl, Mat4 modelMatrix, Boolean moving) {
 ...
 // System.out.println("Amount of lights: " + this.lightArray.size());
 for (int i = 0; i < this.lightArray.size(); i++) {
 ...
 if (textureId1 != null) {
   ...
   gl.glActiveTexture(GL.GL_TEXTURE0);
   gl.glBindTexture(GL.GL_TEXTURE_2D, textureId1[0]);
   //textureId1.bind(gl); // JOGL Texture class needed
 }
 if (textureId2 != null) {
   shader.setInt(gl, "material.specular", 1);
   gl.glActiveTexture(GL.GL_TEXTURE1);
   gl.glBindTexture(GL.GL_TEXTURE_2D, textureId2[0]);
   //textureId2.bind(gl); // JOGL Texture class needed
 }
 mesh.render(gl);
}
